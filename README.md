...
- **📈 Chart templates**: Prefer **`repair_chart` before `read_excel_binary`** (final read), not after every `write_data_to_excel`. Typical case: charts with **dynamic axes** driven by updated data — `write_data_to_excel` / OpenPyXL `save()` can break axis OOXML.
